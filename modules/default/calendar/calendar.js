...
Module.register("calendar", {
      ...
      if (this.config.timeFormat === "dateheaders") {
        ...
        if (event.fullDayEvent) {
          ...
          if (event.today) {
            ...
            if (this.config.timeFormat === "absolute") {
              if (
                ...
                timeWrapper.innerHTML = this.capFirst(
                  moment(event.startDate, "x").from(moment().format("YYYYMMDD"))
                );
              } else {
                timeWrapper.innerHTML = this.capFirst(
          ...
          if (event.startDate >= new Date()) {
            if (event.startDate - now < 2 * oneDay) {
              ...
              if (event.startDate - now < this.config.getRelative * oneHour) {
                ...
                if (
                  this.config.timeFormat === "absolute" &&
                  !this.config.nextDaysRelative
                ) {
                  timeWrapper.innerHTML = this.capFirst(
          ...
          if (this.config.showEnd) {
            timeWrapper.innerHTML += "-";
            timeWrapper.innerHTML += this.capFirst(
              moment(event.endDate, "x").format(this.config.dateEndFormat)
            );
          }
        }
      ...
      for (var e in calendar) {
        ...
        var maxCount =
          Math.ceil(
            (event.endDate -
              1 -
              moment(event.startDate, "x").endOf("day").format("x")) /
              (1000 * 60 * 60 * 24)
          ) + 1;
        if (this.config.sliceMultiDayEvents && maxCount > 1) {
          var splitEvents = [];
          var midnight = moment(event.startDate, "x")
            .clone()
            .startOf("day")
            .add(1, "day")
            .format("x");
  ...
  addCalendar: function (url, auth, calendarConfig) {
    this.sendSocketNotification("ADD_CALENDAR", {
      ...
      auth: auth,
      broadcastPastEvents:
        calendarConfig.broadcastPastEvents || this.config.broadcastPastEvents,
    });
  },
